...
- **Schema validation turns runtime bugs into build errors.** A `z.enum()` on the path field means an invalid value fails `npm run build` with a clear error, not a blank or broken page in production. This is especially valuable for content that non-developers might edit.
- **Individual files per item enable git-level tracking.** When each command is its own markdown file, you get per-command history, blame, and diffs. A single JSON array loses this granularity — "changed commands.json" doesn't tell you which command changed.
- **The break-even point is around 5 items.** With 3 items, the schema definition + collection config + query code is more work than just writing HTML. With 8 items, the abstraction pays for itself in consistency and maintainability. The artifacts collection (3 items) was borderline — justified mainly by the shared pattern with commands.
...
- **Content collections are overkill for pages.** The "Why Stages" and "Getting Started" pages are one-off content without repeated structure. Making them collections would add overhead with no benefit. The distinction is: repeated-structure reference content vs. one-off prose pages.

## Examples

**Good use of content collection (8 items, same shape):**
```
src/content/commands/draft-user.md
src/content/commands/draft-pdr.md
src/content/commands/gen-pdr.md
...
```

**Overkill (1 item, unique shape):**
```
src/content/about/why-stages.md  ← just make this a page
```
...
- [Scaffold-First Static Sites](scaffold-first-static-sites.md) — collections are added in later phases, not during scaffold
- [Phased Implementation with Plan-as-State](phased-implementation-with-plan-as-state.md) — the phased approach that introduced collections incrementally

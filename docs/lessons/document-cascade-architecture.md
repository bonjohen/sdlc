...
- **"Extract, don't invent" is the key discipline.** Draft prompts extract and structure what the user said. They do not add features or requirements the user didn't discuss. Generation prompts make engineering decisions but flag assumptions. This split prevents both under- and over-specification.

## Examples

**Gap surfacing (good):**
```
⚠️ CRITICAL GAP: No pagination strategy defined.
User requirements mention "browse all commands" (US-007) but do not
specify behavior when commands exceed one screen. PDR Section 4.2
assumes a single scrollable list. If the command set grows beyond
~20 items, this will need revisiting.
```

**Silent interpolation (bad):**
```
## Pagination
The system shall display 25 commands per page with next/previous
navigation and a page-size selector.
```
(Nobody asked for this. It looks authoritative but was invented.)
...
- [Prompt Pipeline as Product](prompt-pipeline-as-product.md) — the pipeline architecture that makes the cascade possible
- [Draft-to-Final Document Pipeline](draft-to-final-document-pipeline.md) — the quality gate between draft and final stages of the cascade
